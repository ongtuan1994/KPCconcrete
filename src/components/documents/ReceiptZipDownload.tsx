...
/** Sequentially snapshots each receipt's printable doc-sheet to a PDF blob,
    bundles them into a single ZIP, then triggers a browser download. */
export function ReceiptZipDownload({
  receipts,
  onProgress,
  onDone,
}: {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        onProgress?.(blobsRef.current.length, receipts.length)
      } catch (err) {
  ...
  }, [idx, receipts, onProgress])

  useEffect(() => {
    if (idx < receipts.length || finishedRef.current) return
    ...
    ;(async () => {
      try {
        ...
        a.href = url
        a.download = `receipts-${today}.zip`
        document.body.appendChild(a)
        a.click()
        a.remove()
        setTimeout(() => URL.revokeObjectURL(url), 1500)
      } catch (err) {
        console.error('Receipt ZIP build failed', err)
      } finally {
        onDone()
      }
    })()
  }, [idx, receipts.length, onDone])
