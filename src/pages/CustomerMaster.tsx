...
import { baht, qm, prodShort, customerAgg, customerPerson, customerUnit } from '../data/selectors'
...
/** ชื่อลูกค้า (grouping key) and หน่วยงาน. For seed records that predate the split,
    these derive the parts from the combined `name` (split at the first space / "/"). */
const personName = (c: Customer) => customerPerson(c)
const unitName = (c: Customer) => customerUnit(c)

/** Display row: `first` is true on the first row of each ชื่อลูกค้า group, so the
    table can de-emphasise the repeated customer name on its other หน่วยงาน rows. */
type Row = Customer & { first: boolean }
...
export function CustomerMaster() {
  ...
  const rows: Row[] = useMemo(() => {
    const filtered = list.filter((c) => {
      ...
      if (query) {
        ...
        if (!`${c.name} ${c.legalName} ${c.taxId} ${c.phone ?? ''} ${c.unit ?? ''}`.toLowerCase().includes(q)) return false
      }
      return true
    }).sort((a, b) => {
      /* Group by ชื่อลูกค้า so a customer's หน่วยงาน sit together; within one
         customer, the biggest spender comes first. */
      const pa = personName(a), pb = personName(b)
      if (pa !== pb) return pa.localeCompare(pb, 'th')
      return (aggOf(b.name)?.sales ?? 0) - (aggOf(a.name)?.sales ?? 0)
    })
    let prev: string | null = null
    return filtered.map((c) => {
      const pn = personName(c)
      const first = pn !== prev
      prev = pn
      return { ...c, first }
    })
  ...
  /* Distinct customers (ชื่อลูกค้า) — one customer may own several หน่วยงาน rows. */
  const customerCount = new Set(list.map(personName)).size
  ...
  const columns: Column<Row>[] = [
    { key: 'id', header: 'รหัส', cell: (r) => r.id, className: 'docno' },
    {
      ...
      cell: (r) => (
        <div className="stack" style={{ gap: 2 }}>
          {/* Repeated customer name is faded so grouped หน่วยงาน read as one customer. */}
          <span style={{ color: r.first ? 'var(--kpc-text-strong)' : 'var(--kpc-text-faint)' }}>{personName(r)}</span>
    ...
    },
    {
      ...
      header: 'หน่วยงาน',
      cell: (r) => (unitName(r)
        ? <span className="th">{unitName(r)}</span>
  ...
  return (
    <>
        ...
        actions={
          <>
            <Button variant="secondary" onClick={() => {
              const head = ['รหัส', 'ชื่อลูกค้า', 'หน่วยงาน', 'ชื่อนิติบุคคล', 'เบอร์ติดต่อ', 'ประเภท', 'เงื่อนไข', 'เครดิต (วัน)', 'วงเงินเครดิต', 'เลขผู้เสียภาษี', 'ที่อยู่']
              const body = rows.map((c) => [
                c.id, personName(c), unitName(c), c.legalName ?? '', c.phone ?? '', c.type, c.terms,
      ...
      <div className="grid g-4" style={{ marginBottom: 24 }}>
        <KpiCard label="ลูกค้าทั้งหมด · Customers" value={customerCount.toString()} note={`${list.length} หน่วยงาน`} />
    ...
    <Modal open={!!customer} title={`ลูกค้า ${customer.id}`} onClose={onClose} maxWidth={640}
      ...
      <div className="stack" style={{ gap: 18 }}>
        <div className="stack" style={{ gap: 4 }}>
          <span style={{ fontSize: 18, fontWeight: 600, color: 'var(--kpc-text-strong)' }}>{personName(customer)}</span>
          <span style={{ fontSize: 13, color: 'var(--kpc-text-muted)' }}>
            หน่วยงาน: {unitName(customer) || '—'}{customer.legalName ? ` · ${customer.legalName}` : ''}
          </span>
        </div>
...
function CustomerEditForm({ customer, onClose }: { customer: Customer | null; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!customer) return
    setCustomerName(customerPerson(customer))
    setUnit(customerUnit(customer))
  ...
  const save = () => {
    const edit: CustomerEdit = {
      /* `customerName`/`unit` re-group the record for display; the join key
         `customer.name` is intentionally left untouched so existing documents
         that reference it stay linked. */
  ...
  return (
    ...
    >
      ...
      <div className="grid g-2" style={{ gap: 12, marginBottom: 12 }}>
        <Field label="ชื่อลูกค้า" hint="ใช้จัดกลุ่มลูกค้า — ลูกค้าเดียวกันใช้ชื่อเดียวกัน">
